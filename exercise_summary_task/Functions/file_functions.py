...
def read_from_json(file_path:str,file_name:str)->pd.DataFrame:
    """Function reads json file and returns it a dataframe format"""
    return pd.read_json(file_path+"/"+file_name)

def read_from_csv(file_path:str,file_name:str)->pd.DataFrame:
    """Function reads csv file and returns it a dataframe format"""
    return pd.read_csv(file_path+"/"+file_name)

def write_to_parquet(data_frame:pd.DataFrame,file_path:str,file_name:str=None):
